# 🗂️ Rastros del 36
### Big Data aplicado a la Memoria Histórica de España

Proyecto de análisis de datos sobre la represión franquista y el exilio 
republicano en España. Combina técnicas de Big Data con criterio archivístico 
para construir una plataforma de memoria histórica.

---

## 📊 Fase 1 · Víctimas mortales de la Guerra Civil en Euskadi

Análisis exploratorio de **21.367 víctimas nominales** de la Guerra Civil 
en el País Vasco, a partir del dataset de Open Data Euskadi.

### Principales hallazgos
- **3.202 personas ejecutadas** extrajudicialmente o por Consejo de Guerra
- **2.355 muertes en cautividad**
- **1.471 víctimas de bombardeos**
- **300 profesiones distintas** · perfil mayoritario: jornalero y agricultor
- **109 filiaciones políticas** · predominio de UGT, CNT y PNV
- Fallecimientos registrados en **58 provincias** de toda España

### Contenido
| Archivo | Descripción |
|---|---|
| `fase1_victimas_euskadi.ipynb` | Notebook completo con análisis y visualizaciones |
| `mapa_victimas_euskadi.html` | Mapa interactivo de fallecimientos por provincia |
| `victimas_guerra_civil.xlsx` | Dataset original — Open Data Euskadi |
| `fase2_integracion_nacional.ipynb` | Integración Euskadi + Andalucía. Mapa nacional combinado |
| `mapa_fosas_andalucia.html` | Mapa interactivo de fosas en Andalucía |
| `mapa_nacional_fase2.html` | Mapa nacional combinado Euskadi + Andalucía |
| `mapa_fosas_victimas_guerra_civil_andalucia.csv` | Dataset fosas Andalucía — Junta de Andalucía |
---

## 🗺️ Hoja de ruta del proyecto

- [x] **Fase 1** · Análisis de víctimas en Euskadi
- [x] **Fase 2** · Integración Andalucía y mapa nacional
- [ ] **Fase 3** · Cruce con registros del exilio republicano
- [ ] **Fase 4** · Modelo de datos relacional y grafo de conocimiento
- [ ] **Fase 5** · Plataforma pública con buscador y visualizaciones

---

## 🛠️ Tecnologías utilizadas
Python · Pandas · Matplotlib · Seaborn · Folium · Jupyter Notebook

## 📂 Fuentes de datos
- [Open Data Euskadi](https://www.euskadi.eus) — Gobierno Vasco
- Ministerio de Presidencia — Mapa Estatal de Fosas *(próximas fases)*
- PARES — Portal de Archivos Españoles *(próximas fases)*

## 👩‍💻 Autora
Marisa Lozano · Archivera y analista de datos  
https://www.linkedin.com/in/marisa-lozano-arroyo-2a8a2072/ · marisalozan@gmail.com

---
*Este proyecto nace de la convicción de que los datos pueden ser 
una herramienta de justicia y memoria.*
